...
def main():
    ...
    transformerLAB = FunctionTransformer(rgb2lab)
    transformerHSV = FunctionTransformer(rgb2hsv)


    bayes_rgb_model = make_pipeline(GaussianNB())
    bayes_convert_model = make_pipeline(transformerHSV,GaussianNB())

    knn_rgb_model = make_pipeline(KNeighborsClassifier(n_neighbors=(len(pd.unique(y))-3)))
    knn_convert_model = make_pipeline(transformerLAB,KNeighborsClassifier(n_neighbors=(len(pd.unique(y))-3)))

    rf_rgb_model = make_pipeline(RandomForestClassifier(n_estimators=100, criterion='gini'))
    rf_convert_model = make_pipeline(transformerLAB,RandomForestClassifier(n_estimators=100, criterion='gini'))
