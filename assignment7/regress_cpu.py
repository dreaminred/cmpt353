...
def get_data(filename):
    ...
    #sysinfo['timestamp_num'] = sysinfo['timestamp'].apply(datetime.timestamp)
    #sysinfo['timestamp_num'] = sysinfo['timestamp_num'] - sysinfo['timestamp_num'].min()
    sysinfo['temperature_next'] = sysinfo['temperature'].shift(-1) # should be the temperature value from the next row
    sysinfo = sysinfo[sysinfo['temperature_next'].notnull()] # the last row should have y_column null: no next temp known
    #sysinfo = sysinfo.drop(columns=['timestamp'],axis=1)
...
def output_regression(coefficients, X_columns):
...
def smooth_test(coef, sysinfo, outfile):
    X_valid, y_valid = sysinfo.iloc[:,1:-1], sysinfo.iloc[:,-1]
    
    # feel free to tweak these if you think it helps.
    transition_stddev = 0.5
    observation_stddev = 1.9
...
def main(training_file, validation_file):
    ...
    output_regression(coefficients, X_train)
