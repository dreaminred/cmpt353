...
def pivot_months_pandas(data):
    ...
    """
    # Change from mm/dd/yyyy to yyyy-mm and append to the end
    data['month'] = data['date'].apply(date_to_month)

    # Aggregating over name and month columns
    grouped_data = data.groupby(['name', 'month'])

    # Getting the total for each month
    aggregate_data = grouped_data.aggregate('sum').reset_index()

    # Creating a row for each city and a row for each month
    monthly = aggregate_data.pivot(index='name', columns='month', values='precipitation')

    # Grabbing the monthly counts and creating pivot table
    grouped_data_df = grouped_data.describe()
    counts = grouped_data_df[('precipitation')].reset_index().pivot(index='name', columns='month', values='count')
